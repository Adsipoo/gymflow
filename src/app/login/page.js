    ...
    <div style={{
      ...
      <div style={{ maxWidth: 400, width: '100%' }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <div style={{ fontSize: 32, fontWeight: 700, color: '#1C1C1E', letterSpacing: -0.5, lineHeight: 1.2 }}>
            {mode === 'login' ? 'Welcome back' : 'Create account'}
          </div>
          <div style={{ width: 32, height: 2, background: '#007AFF', margin: '16px auto', borderRadius: 1 }} />
        </div>
        ...
        <div style={{
          ...
          <button onClick={handleGoogle} disabled={loading} style={{
            ...
            gap: 10, fontFamily: 'inherit', marginBottom: 24,
          }}>
          ...
          <form onSubmit={handleEmail} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            {mode === 'signup' && (
              <input type="text" placeholder="Full name" value={fullName}
                ...
                style={{ padding: '14px 16px', borderRadius: 12, border: '1px solid rgba(0,0,0,0.08)', background: '#F2F2F7', color: '#1C1C1E', fontSize: 15, outline: 'none', fontFamily: 'inherit' }}
              />
            )}
            ...
            {error && (
              <div style={{ color: '#FF3B30', fontSize: 13, padding: '10px 14px', background: '#FF3B3010', borderRadius: 10 }}>
                {error}
              </div>
            )}

            <button type="submit" disabled={loading} style={{
              padding: 16, borderRadius: 14, border: 'none', background: '#007AFF',
              color: '#fff', fontSize: 16, fontWeight: 600, cursor: 'pointer',
              fontFamily: 'inherit', marginTop: 4, opacity: loading ? 0.6 : 1,
            }}>
