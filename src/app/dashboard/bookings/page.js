...
export default function BookingsPage() {
  ...
  useEffect(() => {
    async function load() {
      const [bkRes, clsRes, trRes, rvRes] = await Promise.all([
  ...
  const hasReviewed = (classId) => reviews.some(r => r.class_id === classId)
  ...
  return (
    <div style={{ padding: '24px 20px 100px', maxWidth: 680, margin: '0 auto' }}>
      ...
      {active.length > 0 && (
        <>
          <h3 style={{ color: '#8E8E93', fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 10 }}>
            Upcoming
          </h3>
          {active.map(bk => {
              ...
              <div key={bk.id} style={{
                ...
                <div>
                  <div style={{ display: 'flex', gap: 6, marginBottom: 6 }}>
                    ...
                    <Bdg text="Booked" color="#34C759" />
                  </div>
                  <h3 style={{ color: '#1C1C1E', margin: '0 0 4px', fontSize: 15, fontWeight: 600 }}>
                    {tr.name}
                  </h3>
      ...
      {completed.length > 0 && (
        <>
          <h3 style={{ color: '#8E8E93', fontSize: 13, fontWeight: 600, textTransform: 'uppercase', letterSpacing: 0.5, margin: '24px 0 10px' }}>
            Completed
          </h3>
          {completed.map(bk => {
            const cls = getClass(bk.class_id)
        ...
        <>
          ...
          {cancelled.slice(0, 5).map(bk => {
            ...
            return (
              <div key={bk.id} style={{
                background: '#FFFFFF', border: '1px solid rgba(0,0,0,0.06)',
                borderRadius: 14, padding: 18, marginBottom: 8, opacity: 0.5,
                boxShadow: '0 1px 3px rgba(0,0,0,0.06)',
              }}>
                <div style={{ display: 'flex', gap: 6, marginBottom: 4 }}>
                  <Bdg text={cls.type} color={tc[cls.type]} />
          ...
          <div onClick={() => { setReviewModal(null); setRevRating(0); setRevComment('') }} style={{
            ...
            <div onClick={e => e.stopPropagation()} style={{
              ...
              <div style={{ textAlign: 'center', marginBottom: 24 }}>
                ...
                <h2 style={{ color: '#1C1C1E', margin: '0 0 4px', fontSize: 20, fontWeight: 700 }}>
                  How was {cls?.type}?
                </h2>
              ...
              <button
                ...
                style={{
                  ...
                  fontSize: 16, fontWeight: 600, cursor: revRating > 0 ? 'pointer' : 'default',
                  fontFamily: 'inherit',
                }}
              >
                Submit Review
